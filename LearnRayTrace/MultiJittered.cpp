	...
	for (int p = 0; p < num_sets; p++)
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++) {
				...
				const int indexA = i * n + j + p * num_samples;
				const int indexB = i * n + k + p * num_samples;

				if (indexA < samples.size() && indexB < samples.size())
				{
					float t = samples[indexA].x();
					samples[indexA].x() = samples[indexB].x();
					samples[indexB].x() = t;
				}
	...
	for (int p = 0; p < num_sets; p++)
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++) {
				...
				const int indexA = j * n + i + p * num_samples;
				const int indexB = k * n + i + p * num_samples;

				if (indexA < samples.size() & indexB < samples.size())
				{
					float t = samples[indexA].y();
					samples[indexA].y() = samples[indexB].y();
					samples[indexB].y() = t;
				}
		}
}